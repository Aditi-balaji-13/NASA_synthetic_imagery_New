...
TRAIN_LIST = [
    "ROSETTA", "AQUA", "ASTRA", "AURA", "DEEP SPACE 1",
    "MARS ODYSSEY", "MESSENGER", "TDRS", "ULYSSES",
    "PIONEER", "JUNO", "MGS", "GALILEO", "NEAR", "IBEX"
]
VAL_LIST = [
    "GRACE", "DAWN", "NPP", "EPOXI", "ACRIMSAT"
]
TEST_LIST = [
    "VOYAGER", "CASSINI 66", "THEMIS", "STARDUST", "MRO"
]


RANDOM_LIGHTING = True
spacecraft_directory = '../data/loadable_spacecraft/'
background_image_directory = '../data/background/'
NUMBER_OF_IMAGES = 500 # how many images to generate
...
def BoundingBox(obj,cam, imageNum):
...
def clear_render_result():
    """Clear the last render result from memory."""
    if "Render Result" in bpy.data.images:
        render_result = bpy.data.images["Render Result"]
        # Unlink the image from all areas (to avoid user count issues)
        for area in bpy.context.window_manager.windows:
            for space in area.screen.areas:
                if space.type == 'IMAGE_EDITOR' and space.spaces.active.image == render_result:
                    space.spaces.active.image = None
        # Clear the users and remove the render result
        render_result.user_clear()
...
def generate(num,
    ...
    with open(spacecraft_dir + "/merged.json", "r") as json_file:
    ...
    for i, frame in enumerate(sequence):
        ...
        frame.background_image = mathutils.Euler([3.1415 / 2, 0, 0])#str(background_image)
        if image_node_in_tree:
            if random_crop: 
...
def main():
    generate(NUMBER_OF_IMAGES, lighting_effect_types, background_image_directory, spacecraft_directory)
